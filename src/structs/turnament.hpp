...
namespace ivl::structs {
  ...
  struct Turnament {
    ...
    Turnament(Rg&& rg, Pl&& plus = Pl{})
      : plus(FWD(plus))
      , length(std::bit_ceil((std::size_t)std::ranges::distance(rg)))
      , data(length * 2)
    {
      std::size_t idx = 0;
      for (auto&& el : FWD(rg)){
    ...
    const T& leaf(std::size_t pos) const {
  ...
  Turnament(Rg&&, Plus&&) ->
    Turnament<std::ranges::range_value_t<Rg>, std::remove_cvref_t<Plus>>;
  template<typename Rg>
  Turnament(Rg&&) ->
    Turnament<std::ranges::range_value_t<Rg>>;
