...
namespace ivl::structs {
  ...
  struct SplayTree {
    ...
    SplayTree(SplayTree&& o) noexcept : root(std::move(o.root)){
      fixup();
    }
    ...
    Side child_side(SplayTree& c) const {
      IVL_DBG_ASSERT(root);
      IVL_DBG_ASSERT(&child(Side::Left) == &c ||
                     &child(Side::Right) == &c);
    ...
    void refresh_state(){
      IVL_DBG_ASSERT(root);
      root->value.refresh_state(child(Side::Left).value_ptr(),
                                child(Side::Right).value_ptr());
    }
    ...
    int max_depth(){
      if (empty()) return 0;
      return 1 + std::max(child(Side::Left).max_depth(),
                          child(Side::Right).max_depth());
    }
