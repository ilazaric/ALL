...
uint64_t from_hex(std::string_view s){
  if (s.size() > 16){std::cerr << "too long hex: " << s << std::endl; exit(1);}
  uint64_t out = 0;
  for (auto c : s) out = out*16 + from_hex(c);
  return out;
}

void do_not_optimize(const auto& value) {
...
int main(int argc, char** argv){
  ...
  auto virt_start = mmap(nullptr, device_end-device_start, PROT_EXEC | PROT_READ | PROT_WRITE, MAP_PRIVATE, fd.get(), 0);
