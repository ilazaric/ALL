...
namespace ivl::alloc {
  ...
  struct SegmentTree2 {
    ...
    static consteval auto garbage() {
      if constexpr (I + 1 == 0) {
        return std::tuple<>();
      } else {
        return std::tuple_cat(garbage<I - 1>(), std::tuple<Packed<(LENGTH >> I), I + 1>>());
      }
    }
    ...
    template <std::size_t Level>
    void refresh_upwards(std::uint32_t idx) {
