...
template <std::size_t LeafCount> struct SegmentTree {
  ...
  struct Node {
    ...
    template <bool Kind> void modify([[maybe_unused]] std::uint32_t len) {
      if constexpr (Kind) {
  ...
  SegmentTree() {
    ...
    for (std::uint32_t idx = LENGTH - 1; idx; --idx)
      data[idx].rebuild(LENGTH / std::bit_floor(idx), data[idx * 2],
                        data[idx * 2 + 1]);
  }
  ...
  template <bool Kind>
  void modify(std::uint32_t idx, std::uint32_t lo, std::uint32_t hi,
              std::uint32_t leftlen) {
    while (lo != hi) {
