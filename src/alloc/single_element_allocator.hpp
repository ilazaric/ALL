...
namespace ivl::alloc {
  ...
  template<typename T, std::uintptr_t Location, std::size_t Size> requires (Size <= (1ULL << 32))
  struct SingleElementAllocator {
    ...
    static auto get_slot(std::uint32_t idx){
      return sea_detail::get_slot<T, Location, Size>(idx);
    }
