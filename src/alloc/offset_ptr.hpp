...
template <typename T, std::uintptr_t Offset,
          typename OffsetType = std::uint32_t>
struct offset_ptr {
private:
  OffsetType offset;
...
public:
  ...
  template <typename U>
  // prevents int -> char
  requires(std::is_same_v<std::remove_const_t<T>, std::remove_const<U>> ||
           std::is_void_v<T> ||
           std::is_void_v<U>) explicit(std::is_void_v<U> && !std::is_void_v<T>)
      offset_ptr(offset_ptr<U, Offset, OffsetType> o) noexcept
  ...
  reference operator*() const requires(!std::is_void_v<element_type>) {
  ...
  pointer operator->() const requires(!std::is_void_v<element_type>) {
    return &**this;
  }

  offset_ptr &operator++() requires(!std::is_void_v<element_type>) {
    offset += sizeof(T);
    return *this;
  }
  ...
  offset_ptr operator++(int) requires(!std::is_void_v<element_type>) {
    auto ret = *this;
    offset += sizeof(T);
    return ret;
  }
  ...
  offset_ptr &
  operator+=(difference_type delta) requires(!std::is_void_v<element_type>) {
    offset += static_cast<OffsetType>(delta);
    return *this;
  }

  friend offset_ptr
  operator+(offset_ptr ptr,
            difference_type delta) requires(!std::is_void_v<element_type>) {
    ptr += delta;
    return ptr;
  }
  ...
  operator+(difference_type delta,
            offset_ptr ptr) requires(!std::is_void_v<element_type>) {
    ptr += delta;
    return ptr;
  }
  ...
  static offset_ptr
  pointer_to(element_type &el) requires(!std::is_void_v<element_type>) {
