        ...
        for (size_t idx = 0; idx < nonstatic_data_members_of(^T).size(); ++idx){
          if ((false || ... || (idx == nsdm_idxs))){
            ...
            for (auto [tuple_idx, nsdm_idx] : std::vector<size_t>{nsdm_idxs...} | std::views::enumerate){
              if (nsdm_idx < idx){
                left_idxs = ^{ \tokens(left_idxs) , \(nsdm_idx)};
                left_tuple_gets = ^{ \tokens(left_tuple_gets) std::get<\(tuple_idx)>(FWD(self).data), };
              } else {
                right_idxs = ^{ \tokens(right_idxs) , \(nsdm_idx)};
                right_tuple_gets = ^{ \tokens(right_tuple_gets) std::get<\(tuple_idx)>(FWD(self).data), };
              }
            }
            queue_injection(^{
