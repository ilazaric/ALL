...
private:
    ...
    template<std::size_t> struct IndexToTypes;
    template<typename...> struct TypesToIndex;
...
public:
    ...
    static consteval std::vector<std::meta::info> get(std::size_t idx){
        assert(idx < length());
        auto slot = substitute(^IndexToTypes, {std::meta::reflect_value(idx)});
    ...
    static consteval void push(std::vector<std::meta::info> is){
        ...
        Inc::advance();
        define_class(
            substitute(^IndexToTypes, {std::meta::reflect_value(idx)}),
        ...
        define_class(
            ...
            {std::meta::nsdm_description(
                substitute(^Index, {std::meta::reflect_value(idx)})
            )}
        );
    }
...
template<typename A, typename B> using Drop = B;
...
struct Callable {
    ...
    void operator()(auto&& ... args) requires((TL::push(std::vector{^decltype(args)...}), true)){
