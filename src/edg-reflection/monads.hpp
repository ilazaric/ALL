...
namespace ivl::refl {
  ...
  shallow_public_member_functions_of(std::meta::info type){
  ...
  consteval std::vector<std::meta::info>
  deep_public_member_functions_of(std::meta::info type){
    if (!type_is_class(type)) return {};
    std::vector<std::meta::info> ret;
  ...
  consteval void monad_complete(std::meta::info type, std::meta::info under){
    ...
    for (auto memfn : memfns) names.push_back(identifier_of(memfn));
  ...
  struct Opt : std::optional<T> {
    ...
    constexpr auto monad_bind(this auto&& self, auto&& fn)
      -> decltype(FWD(fn)(FWD(self)))
