...
namespace ivl::langs::tiny {
  ...
  EvalValue evaluate(const UnaryExpression& expr, const EvalState& state) {
    ...
    for (auto op : expr.operators) {
      op.with(
        util::Overload{
          [&](op_not) { ret = !std::get<bool>(ret.kind); }, [&](op_minus) { ret = -std::get<int64_t>(ret.kind); },
  ...
  EvalValue evaluate(const FactorExpression& expr, const EvalState& state) {
    ...
    for (size_t i = 0; i < expr.operators.size(); ++i) {
      ...
      expr.operators[i].with(
        util::Overload{
          [&](op_mul) { a *= b; }, [&](op_div) { a /= b; }, [&](op_mod) { a %= b; },
          [](auto) { throw std::runtime_error("nope"); }
        }
      );
    }
  ...
  EvalValue evaluate(const TermExpression& expr, const EvalState& state) {
    ...
    for (size_t i = 0; i < expr.operators.size(); ++i) {
      ...
      expr.operators[i].with(
        util::Overload{
          [&](op_plus) { a += b; }, [&](op_minus) { a -= b; }, [](auto) { throw std::runtime_error("nope"); }
        }
      );
    }
    return a;
  ...
  EvalValue evaluate(const Expression& expr, const EvalState& state) { return evaluate(expr.expression, state); }
