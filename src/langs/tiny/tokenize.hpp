...
namespace ivl::langs::tiny {
  ...
  struct number_start {};
  template<int Digit> struct number_digit {};
  struct number_end {};

  struct identifier_start {};
  template<char Char> struct identifier_char {};
  struct identifier_end {};

  template<auto, auto, template<auto> typename> struct tl_range;
  template<auto L, template<auto> typename TT>
  struct tl_range<L, L, TT> { using type = meta::tl<TT<L>>; };
  ...
  using number_tokens = meta::tl_concat<
    meta::tl<number_start, number_end>,
    tl_range<0, 9, number_digit>::type
    >::type;

  using identifier_tokens = meta::tl_concat<
    meta::tl<identifier_start, identifier_end>,
    tl_range<'a', 'z', identifier_char>::type,
    tl_range<'A', 'Z', identifier_char>::type,
    tl_range<'0', '9', identifier_char>::type
    >::type;

  struct not_a_token {};

  using all_tokens = meta::tl_concat<
    meta::tl<not_a_token>,
    kw_tokens,
    op_tokens,
    number_tokens,
  ...
  struct Token {
    ...
    template<typename T> requires(meta::tl_contains<T, all_tokens>::value)
    bool has() const {
  ...
  std::vector<Token> tokenize(std::string_view file){
    ...
    while (!file.empty()){
      if (isspace(file[0])){file.remove_prefix(1); continue;}

      // all ops
#define X(str, type) if (file.starts_with(str)){out.emplace_back(type{}); file.remove_prefix(std::string_view(str).size()); continue;}
      X("||", op_or);
      X("&&", op_and);
      X("==", op_eq);
      X("!=", op_ne);
