...
namespace ivl::langs::tiny {
  ...
  struct AndExpression { std::vector<EqualsExpression> expressions; };
  struct OrExpression { std::vector<AndExpression> expressions; };
  struct Expression { OrExpression expression; };
  ...
  struct ParserState {
    ...
    bool has() const { return !empty() && (false || ... || token_storage[current].has<Ts>()); }
    template<typename T>
    bool consume_if() {
    ...
    void pop() { assert(!empty()); ++current; }
  };

  struct Statement;
  
  struct LetStatement { Identifier identifier; Expression expression; };

  struct AssignStatement { Identifier identifier; Expression expression; };

  struct PrintStatement { Expression expression; };
  ...
  struct BlockStatement { std::vector<Statement> statements; };
  ...
  template<>   Identifier parse(ParserState& state) ;
  template<>   ParenExpression parse(ParserState& state) ;
  template<>   PrimaryExpression parse(ParserState& state) ;
  template<>   UnaryExpression parse(ParserState& state) ;
  template<>   FactorExpression parse(ParserState& state) ;
  template<>   TermExpression parse(ParserState& state) ;
  template<>   ComparisonExpression parse(ParserState& state) ;
  template<>   EqualsExpression parse(ParserState& state) ;
  template<>   AndExpression parse(ParserState& state) ;
  template<>   OrExpression parse(ParserState& state) ;
  template<>   Expression parse(ParserState& state) ;
  template<>   LetStatement parse(ParserState& state) ;
  template<>   AssignStatement parse(ParserState& state) ;
  template<>   PrintStatement parse(ParserState& state) ;
  template<>   IfStatement parse(ParserState& state) ;
  template<>   WhileStatement parse(ParserState& state) ;
  template<>   BlockStatement parse(ParserState& state) ;
  template<>   Statement parse(ParserState& state) ;
  ...
  PrimaryExpression parse(ParserState& state){
    if (state.has<kw_false>()){ state.pop(); return PrimaryExpression{False{}}; }
    if (state.has<kw_true>()){ state.pop(); return PrimaryExpression{True{}}; }
    if (state.has<identifier_start>()){ return PrimaryExpression{parse<Identifier>(state)}; }
  ...
  template<>
  Expression parse(ParserState& state){ return Expression{parse<OrExpression>(state)}; };
  ...
  Statement parse(ParserState& state){
    if (state.has<kw_let>()) return Statement{parse<LetStatement>(state)};
    if (state.has<kw_while>()) return Statement{parse<WhileStatement>(state)};
    if (state.has<kw_if>()) return Statement{parse<IfStatement>(state)};
    if (state.has<op_curly_open>()) return Statement{parse<BlockStatement>(state)};
    if (state.has<kw_print>()) return Statement{parse<PrintStatement>(state)};
