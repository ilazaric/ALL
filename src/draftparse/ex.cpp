...
int main() {
  for (auto d : std::filesystem::directory_iterator{"/home/ilazaric/repos/draft/source"}) {
    auto& f = d.path();
    if (f.extension() != ".tex") continue;
    if (f.filename() == "macros.tex") continue;
    if (f.filename() == "intro.tex") continue;
    LOG(f);
    ...
    for (std::string s; std::getline(fin, s);) {
      ...
      if (auto loc = tail.find('%'); loc != tail.npos) tail = tail.substr(0, loc);

      while (!tail.empty()) {
        auto it  = std::ranges::find_if(tail, std::iswspace);
        auto loc = it == tail.end() ? tail.npos : it - tail.begin();
        if (loc) words.emplace_back(tail.substr(0, loc));
        if (loc == tail.npos) break;
    ...
    for (auto idx : std::views::iota(0ull, words.size())) {
      ...
      if (lo == std::string_view::npos) continue;
      if (hi == std::string_view::npos) continue;
      if (lo > hi) continue;
      // assert(hi != std::string_view::npos);
      // assert(lo < hi);
      auto kind = words[idx].substr(lo + 1, hi - lo - 1);
      if (std::ranges::find(stuff, kind) == stuff.end()) continue;
