...
namespace ivl::fs {
  ...
  struct FileView {
    ...
    FileView(ivl::str::NullStringView path, std::size_t offset)
    ...
    FileView(FD fd, std::size_t offset, std::size_t length) {
      void* mapped_region =
        length ? mmap(0, length, PROT_READ, MAP_PRIVATE, fd.get(), offset) : nullptr;
    ...
    std::span<const T> as_span() {
      ...
      return std::span<const T>(static_cast<const T*>(static_cast<const void*>(mapped_region)),
                                length / sizeof(T));
    }

    std::string_view as_string_view() {
      return std::string_view(static_cast<const char*>(static_cast<const void*>(mapped_region)),
                              length);
    }
