...
#define INDENT std::string(dbg_indent * 2, ' ')
  Latex parse_latex_string(std::string_view& data, const char terminator) {
    ...
    DBG(std::cerr << INDENT << "parsing..."; if (terminator) std::cerr << terminator;
        std::cerr << std::endl;);
  ...
  do {                                                                                             \
    ...
    while (!data.empty()) {
      ...
      if (data[0] == '\\') {
        ...
        while (true) {
          ...
          if (data[0] != '[' && data[0] != '{')
            break;
          char open = data[0];
          if (command.name == "keyword" && open == '[')
            break;
          char close = open == '[' ? ']' : '}';
          data.remove_prefix(1);
          command.args.emplace_back(open == '[' ? LatexArg::Kind::BRACKET : LatexArg::Kind::CURLY,
                                    parse_latex_string(data, close));
      ...
      while (!data.empty() && data[0] != '\\' && data[0] != terminator) {
        skip_comment(data);
        if (!(!data.empty() && data[0] != '\\' && data[0] != terminator))
          break;
        text.text += data[0];
        data.remove_prefix(1);
      }
    ...
    for (size_t idx = 0; idx < latex.elems.size(); ++idx) {
      ...
      if (command->name == "end") {
        ...
        ret.emplace_back(std::string(environment_edge_name(latex.elems[idx])),
                         ...
                         std::get<LatexCommand>(latex.elems[idx].kind), Latex {});
