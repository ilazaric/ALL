...
namespace ivl::cppp {
  ...
  size_t seek_while(std::string_view data, auto&& pred){
    size_t idx = 0;
    while (idx < data.size() && pred(data[idx])) ++idx;
    return idx;
  }

  void skip_whitespace(std::string_view& data){
    data.remove_prefix(seek_while(data, (int(&)(int))&std::isspace));
  }
  ...
  Latex parse_latex_string(std::string_view& data, const char terminator){
    dbg_indent += 1;
    struct reverter { ~reverter(){ dbg_indent -= 1; }} reverter;

    DBG(std::cerr << INDENT << "parsing...";
        if (terminator) std::cerr << terminator;
        std::cerr << std::endl;);

    // if (dbg_indent > 20) exit(1);
    
    if (terminator == '\0') original_data = data;

#define my_assert(...) do { if (!(__VA_ARGS__)){ \
      std::string_view prefix = original_data.substr(0, original_data.size() - data.size());\
      size_t newlinecount = 0;\
      while (true){size_t p = prefix.find('\n'); if (p == std::string_view::npos) break; ++newlinecount; prefix.remove_prefix(p+1);}\
    ...
    while (!data.empty()){
      ...
      if (data[0] == terminator){
        // std::cerr << INDENT << "TERMINATOR" << std::endl;
        break;
      }
      ...
      if (data[0] == '\\'){
        ...
        if (!data.empty()){
          ...
          // my_assert(std::isspace(data[0]) || data[0] == '\\' || data[0] == '[' || data[0] == '{' || !std::isalpha(command.name[0]));
        }
        ...
        while (true){
          ...
          char close = open == '[' ? ']' : '}';
          data.remove_prefix(1);
          command.args.emplace_back(open == '[' ? LatexArg::Kind::BRACKET : LatexArg::Kind::CURLY, parse_latex_string(data, close));
          my_assert(!data.empty());
          my_assert(data[0] == close);
          data.remove_prefix(1);
        }

        // LOG(command.name, command.args.size());
  ...
  std::vector<LatexEnvironment> find_environments(const Latex& latex){
    ...
    for (size_t idx = 0; idx < latex.elems.size(); ++idx){
      ...
      if (command->name == "end"){
        ...
        assert(environment_edge_name(latex.elems[begin_stack.back()]) ==
               environment_edge_name(latex.elems[idx]));
        ret.emplace_back(std::string(environment_edge_name(latex.elems[idx])),
                         std::get<LatexCommand>(latex.elems[begin_stack.back()].kind),
                         std::get<LatexCommand>(latex.elems[idx].kind),
                         Latex{});
