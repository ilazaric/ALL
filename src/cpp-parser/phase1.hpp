...
namespace ivl::cppp {
  ...
  bool is_translation_character(UChar32 codepoint) {
    return unicode_is_scalar_value(codepoint);
  }
  ...
  std::vector<UChar32> phase1(ivl::str::NullStringView path) {
    ...
    while (i < length) {
      ...
      if (codepoint < 0)
        throw std::runtime_error("bad utf8 file");
      if (!is_translation_character(codepoint))
        throw std::runtime_error("not translation character");
