...
struct Nim {
  ...
  auto moves() const {return std::views::iota(0ull, x) | std::views::transform([](std::size_t x){return Nim{x};});}
...
template<>
struct std::hash<Nim> {
  std::size_t operator()(const Nim& o) const noexcept {
    return o.x;
  }
};
...
struct Falling {
  ...
  Falling& operator++(){x -= dx; y -= dy; return *this;}
