...
template <typename> struct to_free_function_pointer_helper : std::false_type {};

// qualifiers to member functions are annoying
#define IVLGEN_PARTIAL(QUALS)                                                  \
  template <typename Class, typename Ret, typename... Args>                    \
  struct to_free_function_pointer_helper<Ret (Class::*)(Args...) QUALS>        \
      : std::true_type {                                                       \
    template <Ret (Class::*mfp)(Args...) QUALS>                                \
    static Ret free_function(                                                  \
        std::conditional_t<std::is_reference_v<Class QUALS>, Class QUALS,      \
...
template <auto mfp> struct to_free_function_pointer {
  using helper = to_free_function_pointer_helper<decltype(mfp)>;
  static_assert(helper::value,
                "template argument not a member function pointer");
  static constexpr auto value = helper::template free_function<mfp>;
};

template <auto mfp>
constexpr auto to_free_function_pointer_v =
    to_free_function_pointer<mfp>::value;
