...
namespace ivl {
  ...
  struct process {
    ...
    linux::or_syscall_error<long> wait() {
      return linux::or_syscall_error<long>(ivl::linux::raw_syscalls::wait4(pid, nullptr, 0, nullptr));
    }
  ...
  struct process_config {
    ...
    linux::or_syscall_error<process> clone_and_exec() const {
      ...
      std::vector<const char*> actual_envp{nullptr};
      long                     actual_err = 0;
      for (size_t i = 0; i < argv.size(); ++i)
        actual_argv[i] = argv[i].c_str();
      using T = std::tuple<const char*, const char* const*, const char* const*, long*>;
      T exec_args{actual_pathname, &actual_argv[0], &actual_envp[0], &actual_err};
      ...
      auto ret = linux::raw_syscalls::fat_clone3(
        &clone3_args, sizeof(clone3_args), &exec_args, +[](void* child_arg) noexcept {
          // TODO: prctl suicide
          auto [pathname, argv, envp, err] = *reinterpret_cast<const T*>(child_arg);
