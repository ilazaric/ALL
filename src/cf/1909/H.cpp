...
Move delta_greedy(const Perm& p) {
  ...
  for (auto i : std::views::iota(0_u32, p.size() - 1)) {
    ...
    if (acc > max_seen.first) max_seen = {acc, i};
  }

  return best_move;
}

std::int64_t cubed(std::int64_t a, std::int64_t b) { return abs(a - b) * (a - b) * (a - b); }

std::int64_t delta_norm3(const Perm& p, std::uint32_t i) {
  return cubed(p[i], i + 1) + cubed(p[i + 1], i) - cubed(p[i], i) - cubed(p[i + 1], i + 1);
}

std::int64_t delta_norm2(const Perm& p, std::uint32_t i) { return (std::int64_t)p[i + 1] - (std::int64_t)p[i]; }

std::int64_t delta_norm(const Perm& p, std::uint32_t i) { return (p[i] < i ? -1 : 1) + (p[i + 1] > i + 1 ? -1 : 1); }

double delta_exp(const Perm& p, std::uint32_t i) {
  return +exp2(abs((std::int64_t)p[i] - (std::int64_t)(i + 1))) +
         exp2(abs((std::int64_t)p[i + 1] - (std::int64_t)(i))) - exp2(abs((std::int64_t)p[i] - (std::int64_t)(i))) -
         exp2(abs((std::int64_t)p[i + 1] - (std::int64_t)(i + 1)));
}

std::uint32_t d(std::uint32_t a, std::uint32_t b) { return a > b ? a - b : b - a; }
...
std::strong_ordering cmp(const std::map<K, V>& a, const std::map<K, V>& b) {
  auto ait = a.rbegin();
  auto bit = b.rbegin();
...
template <typename T>
std::strong_ordering cmp2(auto&& r, const T& a, const T& b) {
...
Move reduce_max_dists_simple_alt(const Perm& p) {
  ...
  for (std::uint32_t parity : {0, 1}) {
    ...
    auto                     get = [&](std::uint32_t x) -> T& {
      x = max_dist - x;
      return suffixf[x / N][x % N];
    };
    std::uint32_t l = parity;
    for (std::uint32_t r = parity + 1; r < p.size(); r += 2) {
      --get(d(p[r], r));
  ...
  for (auto l : std::views::iota(0_u32, p.size())) {
    ...
    for (auto r : std::views::iota(l + 1, p.size()))
      if (l % 2 != r % 2) {
        ...
        if (curr_count == best_count) ++best_total;
      }
  broken_exit:;
  }
...
int main() {
...

  {
    ...
    for (auto ti : std::views::iota(1)) {
      ...
      {
        SCOPE_TIMER;
        LOG(p.size(), evaluate(p, reduce_max_dists_attempt));
      }
