...
namespace ivl::proc {
  ...
  struct CtxtSwitchCounts {
    ...
    static CtxtSwitchCounts self() {
      ...
      //       for (auto ptr_begin = data, ptr_end = data; ptr_begin != data + read_cnt + 1;
      //       ptr_begin = ptr_end + 1){
      //         ptr_end = strchr(ptr_begin, '\n');
      //         *ptr_end = '\0';
      // #define BLA(str, kind) if (strncmp(str, ptr_begin, sizeof(str) - 1) == 0) ret.kind =
      // atoi(ptr_begin + sizeof(str) + 1) #define TRUC(kind) BLA(#kind "_ctxt_switches", kind)
      //         TRUC(voluntary);
      //         TRUC(nonvoluntary);
      // #undef BLA
      // #undef TRUC
