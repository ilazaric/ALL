  ...
  while (!WindowShouldClose()) {
    ...
    for (auto& drone : drones) {
      if (drone.state == Drone::State::MOVE) {
        ...
        const float d   = sqrtf(
          (mine_pos.x - drone.pos.x) * (mine_pos.x - drone.pos.x) +
          (mine_pos.y - drone.pos.y) * (mine_pos.y - drone.pos.y)
        );
      ...
      if (drone.state == Drone::State::MOVEI) {
        const float ldt = std::min(dt, drone.rem_time);
        const float d   = sqrtf(
          (base_pos.x - drone.pos.x) * (base_pos.x - drone.pos.x) +
          (base_pos.y - drone.pos.y) * (base_pos.y - drone.pos.y)
        );
