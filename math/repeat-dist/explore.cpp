...
Real f(Real x, Real p) { return piecewise(x, p, p * p); }

Real finv(Real x, Real p) { return piecewise(x, p * p, p); }

std::mt19937 gen(13337);
Real         rnd() { return std::uniform_real_distribution<Real>(0, 1)(gen); }
...
int main() {
  ...
  if (0) {
    ...
    while (bounds.size() + 1 < N)
      bounds.push_back(std::sqrt(bounds.back()));
...

  {
    ...
    for (int i = 0;; ++i) {
      if (LOG(i, optimize(i % (N - 1))) < 1e-4) break;
    }
