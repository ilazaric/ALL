...
namespace __format {
  ...
  struct __formatter_int {
    ...
    constexpr __formatter_int(_Spec<_CharT> __spec) noexcept : _M_spec(__spec) {
      if (_M_spec._M_type == _Pres_none) _M_spec._M_type = _Pres_d;
    }
    ...
    template<typename _Tp>
    constexpr typename basic_format_parse_context<_CharT>::iterator _M_parse(basic_format_parse_context<_CharT>& __pc) {
      if constexpr (is_same_v<_Tp, bool>) {
        auto __end = _M_do_parse(__pc, _AsBool);
        if (_M_spec._M_type == _Pres_s)
          if (_M_spec._M_sign != _Sign_default || _M_spec._M_alt || _M_spec._M_zero_fill)
            __throw_format_error(
        ...
        if (_M_spec._M_type == _Pres_c)
          if (
            _M_spec._M_sign != _Sign_default || _M_spec._M_alt || _M_spec._M_zero_fill
        ...
        return __end;
      } else return _M_do_parse(__pc, _AsInteger);
    }
    ...
    format(_Int __i, basic_format_context<_Out, _CharT>& __fc) const {
      if (_M_spec._M_type == _Pres_c) return _M_format_character(_S_to_character(__i), __fc);
      ...
      if (__i < 0) __u = -static_cast<make_unsigned_t<_Int>>(__i);
      else __u = __i;
      ...
      case _Pres_X:
        ...
        if (_M_spec._M_type == _Pres_X)
          for (auto __p = __start; __p != __res.ptr; ++__p) *__p = __format::__toupper_numeric(*__p);
        break;
      }
      ...
      if constexpr (is_same_v<char, _CharT>) return _M_format_int(__narrow_str, __prefix_len, __fc);
    }

    template<typename _Out>
    constexpr typename basic_format_context<_Out, _CharT>::iterator
    format(bool __i, basic_format_context<_Out, _CharT>& __fc) const {
      if (_M_spec._M_type == _Pres_c) return _M_format_character(static_cast<unsigned char>(__i), __fc);
      if (_M_spec._M_type != _Pres_s) return format(static_cast<unsigned char>(__i), __fc);
      ...
      if (_M_spec._M_localized) [[unlikely]] {
        ...
        if constexpr (is_same_v<char, _CharT>) __s = __i ? "true" : "false";
        else __s = __i ? L"true" : L"false";
        __est_width = __s.size();
      }

      return __format::__write_padded_as_spec(__s, __est_width, __fc, _M_spec);
    }

    template<typename _Out>
    constexpr typename basic_format_context<_Out, _CharT>::iterator
    _M_format_character(_CharT __c, basic_format_context<_Out, _CharT>& __fc) const {
      basic_string_view<_CharT> __in(&__c, 1u);
      size_t __width = 1u;

      if constexpr (sizeof(_CharT) > 1u && __unicode::__literal_encoding_is_unicode<_CharT>())
        __width = __unicode::__field_width(__c);

      if (!_M_spec._M_debug) return __format::__write_padded_as_spec(__in, __width, __fc, _M_spec);

      __width += 2;
      if (_M_spec._M_get_width(__fc) <= __width) return __format::__write_escaped(__fc.out(), __in, _Term_apos);
      ...
      __in = __sink.view();
      if (__in[1] == _Escapes<_CharT>::_S_bslash()[0]) __width = __in.size();
      return __format::__write_padded_as_spec(__in, __width, __fc, _M_spec);
    }

    template<typename _Int>
    static constexpr _CharT _S_to_character(_Int __i) {
      using _Traits = __gnu_cxx::__int_traits<_CharT>;
      if constexpr (is_signed_v<_Int> == is_signed_v<_CharT>) {
        if (_Traits::__min <= __i && __i <= _Traits::__max) return static_cast<_CharT>(__i);
      } else if constexpr (is_signed_v<_Int>) {
        if (__i >= 0 && make_unsigned_t<_Int>(__i) <= _Traits::__max) return static_cast<_CharT>(__i);
      } else if (__i <= make_unsigned_t<_CharT>(_Traits::__max)) return static_cast<_CharT>(__i);
      __throw_format_error(
    ...
    template<typename _Out>
    constexpr typename basic_format_context<_Out, _CharT>::iterator _M_format_int(
      basic_string_view<_CharT> __str, size_t __prefix_len, basic_format_context<_Out, _CharT>& __fc
    ) const {
      ...
      if (_M_spec._M_localized) {
        ...
        if (__l.name() != "C") {
          ...
          if (!__grp.empty()) {
            ...
            auto __p = (_CharT*)__builtin_alloca(2 * __n * sizeof(_CharT) + __prefix_len);
            auto __s = __str.data();
            char_traits<_CharT>::copy(__p, __s, __prefix_len);
            __s += __prefix_len;
            auto __end =
              std::__add_grouping(__p + __prefix_len, __np.thousands_sep(), __grp.data(), __grp.size(), __s, __s + __n);
            __str = {__p, size_t(__end - __p)};
          }
        }
      }

      if (__width <= __str.size()) return __format::__write(__fc.out(), __str);
      ...
      if (__align == _Align_default) {
        ...
        if (_M_spec._M_zero_fill) {
          ...
          if (__prefix_len != 0) {
            __out = __format::__write(std::move(__out), __str.substr(0, __prefix_len));
            __str.remove_prefix(__prefix_len);
          }
        } else __fill_char = _CharT(' ');
      }
      return __format::__write_padded(std::move(__out), __str, __align, __nfill, __fill_char);
    }
...
#undef _GLIBCXX_FORMAT_F128
  ...
  struct __formatter_fp {
    ...
    format(_Fp __v, basic_format_context<_Out, _CharT>& __fc) const {
      ...
      if (__use_prec) __prec = _M_spec._M_get_precision(__fc);
      ...
      switch (_M_spec._M_type) {
      case _Pres_P:
        if (__builtin_isfinite(__v)) __offset += 2;
        [[fallthrough]];
      case _Pres_A:
        __upper = true;
        __expc = 'P';
      ...
      default:
      case _Pres_none:
        if (__use_prec) __fmt = chars_format::general;
        break;
      }
      ...
      auto __to_chars = [&](char* __b, char* __e) {
        if (__use_prec) return __format::to_chars(__b, __e, __v, __fmt, __prec);
        else if (__fmt != chars_format{}) return __format::to_chars(__b, __e, __v, __fmt);
        else return __format::to_chars(__b, __e, __v);
      };
      ...
      if (__builtin_expect(__res.ec == errc::value_too_large, 0)) {
        ...
        if (__fmt == chars_format::fixed) {
          if constexpr (is_same_v<_Fp, float> || is_same_v<_Fp, double> || is_same_v<_Fp, long double>) {
            int __exp{};
            if constexpr (is_same_v<_Fp, float>) __builtin_frexpf(__v, &__exp);
            else if constexpr (is_same_v<_Fp, double>) __builtin_frexp(__v, &__exp);
            else if constexpr (is_same_v<_Fp, long double>) __builtin_frexpl(__v, &__exp);
            if (__exp > 0) __guess += 1U + __exp * 4004U / 13301U;
          } else __guess += numeric_limits<_Fp>::max_exponent10;
        }
        ...
        do {
          ...
          __dynbuf.__resize_and_overwrite(__dynbuf.capacity() * 2, __overwrite);
      ...
      if (__offset == 3) {
        __start -= 2;
        if (__builtin_signbit(__v)) ranges::copy(string_view("-0x"), __start);
        else ranges::copy(string_view("0x"), __start);
      }

      if (__upper) {
        for (char* __p = __start; __p != __res.ptr; ++__p) *__p = __format::__toupper_numeric(*__p);
      }

      if (!__builtin_signbit(__v)) {
        if (_M_spec._M_sign == _Sign_plus) *--__start = '+';
        else if (_M_spec._M_sign == _Sign_space) *--__start = ' ';
        else --__offset;
      }
      ...
      if (_M_spec._M_alt && __builtin_isfinite(__v)) {
        ...
        if (__d != __s.npos) {
          ...
          if (__p == __s.npos) __p = __s.size();

          if (__trailing_zeros) {
            if (__s[__offset] != '0') __sigfigs = __p - __offset - 1;
            else __sigfigs = __p - __s.find_first_not_of('0', __d + 1);
          }
        } else {
          __p = __s.find(__expc);
          if (__p == __s.npos) __p = __s.size();
        ...
        if (size_t __extras = int(__d == __p) + __z) {
          if (__dynbuf.empty() && __extras <= size_t(__end - __res.ptr)) {
            __builtin_memmove(__start + __p + __extras, __start + __p, __s.size() - __p);
            if (__d == __p) __start[__p++] = '.';
      ...
      if (__align == _Align_default) {
        ...
        if (_M_spec._M_zero_fill && __builtin_isfinite(__v)) {
          __fill_char = _CharT('0');
          if (__offset > 0) {
            __out = __format::__write(__out, __str.substr(0, __offset));
    ...
    basic_string<_CharT>
    _M_localize(basic_string_view<_CharT> __str, char __expc, int __offset, const locale& __loc) const {
      basic_string<_CharT> __lstr;

      if (__loc == locale::classic()) return __lstr;
      ...
      if (__grp.empty() && __point == __dot) return __lstr;

      size_t __d = __str.find(__dot);
      size_t __e = min(__d, __str.find(__exp));
      if (__e == __str.npos) __e = __str.size();
      const size_t __r = __str.size() - __e;
      auto __overwrite = [&](_CharT* __p, size_t) {
        ranges::copy_n(__str.data(), __offset, __p);

        auto __end = std::__add_grouping(
          __p + __offset, __np.thousands_sep(), __grp.data(), __grp.size(), __str.data() + __offset, __str.data() + __e
        );
        if (__r) {
          if (__d != __str.npos) {
            *__end = __point;
  ...
  struct __formatter_ptr {
    ...
    format(const void* __v, basic_format_context<_Out, _CharT>& __fc) const {
      auto __u = __v ? reinterpret_cast<__UINTPTR_TYPE__>(__v) : static_cast<__UINTPTR_TYPE__>(0);
...
struct formatter<_CharT, _CharT> {
  ...
  format(_CharT __u, basic_format_context<_Out, _CharT>& __fc) const {
    if (_M_f._M_spec._M_type == __format::_Pres_c) return _M_f._M_format_character(__u, __fc);
    else return _M_f.format(static_cast<make_unsigned_t<_CharT>>(__u), __fc);
  }

#if __glibcxx_format_ranges
  constexpr void set_debug_format() noexcept { _M_f._M_spec._M_debug = true; }
#endif
...
struct formatter<_CharT[_Nm], _CharT> {
  ...
  template<typename _Out>
  constexpr typename basic_format_context<_Out, _CharT>::iterator
  format(const _CharT (&__u)[_Nm], basic_format_context<_Out, _CharT>& __fc) const {
...
struct formatter<basic_string<char, _Traits, _Alloc>, char> {
  ...
  [[__gnu__::__always_inline__]]
  constexpr typename basic_format_parse_context<char>::iterator parse(basic_format_parse_context<char>& __pc) {
    return _M_f.parse(__pc);
  }

  template<typename _Out>
  constexpr typename basic_format_context<_Out, char>::iterator
  format(const basic_string<char, _Traits, _Alloc>& __u, basic_format_context<_Out, char>& __fc) const {
    return _M_f.format(__u, __fc);
  }
...
struct formatter<basic_string_view<char, _Traits>, char> {
  ...
  template<typename _Out>
  constexpr typename basic_format_context<_Out, char>::iterator
  format(basic_string_view<char, _Traits> __u, basic_format_context<_Out, char>& __fc) const {
    return _M_f.format(__u, __fc);
  }
