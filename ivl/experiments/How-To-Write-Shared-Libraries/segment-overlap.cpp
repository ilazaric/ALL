...
int main() {
  ...
  const uint64_t code1 = page_size;
  const uint64_t code2 = page_size * 2;
  ...
  {
    elf_header x{};
    x.e_entry = reinterpret_cast<void*>(65536);
    x.e_phoff = sizeof(x);
    x.e_shoff = 0;
...

  {
    ...
    x.p_flags = 5; // XR
    x.p_offset = code1;
    x.p_vaddr = reinterpret_cast<void*>(65536);
    x.p_paddr = x.p_vaddr;
    x.p_filesz = page_size;
    x.p_memsz = 0x1000;
    x.p_align = 1;
    std::string_view sv{(char*)&x, sizeof(x)};
    std::cout << sv;
  ...
  { // exit_group(122)
    /*
      ...
      7:	48 c7 c7 7a 00 00 00 	mov    $0x7a,%rdi
      e:	0f 05                	syscall
     */
    std::string code;
    auto decode = [](char c) { return c >= '0' && c <= '9' ? c - '0' : c - 'a' + 10; };
    std::string rep = "48 c7 c0 e7 00 00 00 48 c7 c7 7a 00 00 00 0f 05";
  ...
  { // exit_group(121)
    /*
      0:	48 c7 c0 e7 00 00 00 	mov    $0xe7,%rax
      7:	48 c7 c7 79 00 00 00 	mov    $0x79,%rdi
      e:	0f 05                	syscall
     */
    std::string code;
    auto decode = [](char c) { return c >= '0' && c <= '9' ? c - '0' : c - 'a' + 10; };
    std::string rep = "48 c7 c0 e7 00 00 00 48 c7 c7 79 00 00 00 0f 05";
    for (size_t i = 0; i < rep.size(); i += 3) {
      code.push_back((char)(decode(rep[i]) * 16 + decode(rep[i + 1])));
    }
    std::cout << code;
