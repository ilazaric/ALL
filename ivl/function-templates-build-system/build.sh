...
echo '#include "fn-template.cpp"' > fn-template.tpp
echo 'template<typename T> using Param = const T&;' >> fn-template.tpp
# objdump -t lib{A,B}.o |
#     grep print |
#     c++filt |
([[ "$FLAGS" == *"-flto"* ]] &&
     lto-dump-11 -list lib{A,B}.o ||
         objdump -t lib{A,B}.o) |
    grep print |
    c++filt |
    sort |
    ...
    cut -d '<' -f 2 |
    cut -d '>' -f 1 |
    awk '{print "template void print<" $0 ">(Param<" $0 ">);"}' >> fn-template.tpp

# fn-template.tpp --> fn-template.o
g++ -xc++ $FLAGS -c fn-template.tpp

# main.cpp --> main.o
g++ $FLAGS -c main.cpp

# lib{A,B}.o, main.o, fn-template.o --> main
g++ $FLAGS lib{A,B}.o main.o fn-template.o -o main
